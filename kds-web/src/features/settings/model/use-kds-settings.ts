...
import { mapApiSettingsToUi, mapUiSettingsToApi } from "../lib/settings-mapper";
...
export function useKdsSettings({ accessToken, onUnauthorized, showToast }: UseKdsSettingsParams) {
  ...
  useEffect(() => {
    void refreshSettings()
      .catch((error) => showToast(error instanceof Error ? error.message : "설정을 불러오지 못했습니다."))
  ...
  return { loading, refreshSettings, saving, settings, updateSettings };
}
