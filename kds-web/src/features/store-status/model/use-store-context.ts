...
export function useStoreContext({ accessToken, onUnauthorized, showToast }: UseStoreContextParams) {
  ...
  async function changeStoreStatus(nextStatus: StoreStatus) {
    ...
    try {
      ...
      if (storeContext) setStoreStatus(storeContext.operatingStatus);
  ...
  function revertPendingPausedStatus() {
    setStoreStatus((currentStatus) => {
      if (currentStatus === "PAUSED" && storeContext?.operatingStatus !== "PAUSED") {
        return storeContext?.operatingStatus ?? "OPEN";
      }
